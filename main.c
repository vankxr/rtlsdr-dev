...
int16_downsampler_t *pRDSBBDownsampler;
int16_downsampler_t *pRDSBitDownsampler;
FILE *pBasebandFile;
FILE *pAudioFile;
FILE *pRDSFile;
...
int16_t am_demod(iq16_t xSample)
{
    ...
    return sqrt(xDiff.i * xDiff.i + xDiff.q * xDiff.q);
}
...
uint8_t stereo_audio_demod(int16_t sBaseband, int16_t sStereoPilot, int16_t *psLeft, int16_t *psRight)
{
    ...
    sStereoPilot >>= 2; // Suppress noise from the recovered carrier, otherwise the L-R will ruin the overall quality
...
void sample_handler(iq16_t xSample)
{
    ...
    // Add LPF here to reduce aliasing from nearby channels
...
int main(int argc, char **argv)
{
    ...
    pRDSBBDownsampler = int16_downsampler_init(RDS_DOWNSAMPLE);
