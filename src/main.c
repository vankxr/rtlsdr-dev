...
#define CHANNEL_CARRIER     (105.6 * 1000000)   // 100.1 MHz - FM channel center frequency
...
#define SQUELCH_LEVEL       -70     // -60 dBFS
...
uint8_t util_fft(iq16_t xSample, uint32_t ulFFTSize, uint32_t ulCenterFrequency, uint32_t ulSampleRate, const char *pszFileName)
{
    ...
    else
    {
        ...
        for(uint32_t i = 0; i < ulFFTSize; i++)
        {
            ...
            fprintf(pFFTFile, "%.6f\t%.2f\t%u\t%.6f\r\n", gFrequency / 1000, gPower, i, (double)pFFTInput[i].i / INT16_MAX);
        }
...
uint8_t rds_demod(int16_t sBaseband, int16_t sRDSPilot, uint8_t *pubBit)
{
    ...
    sRDS = fir_filter(pRDSBandPass, xRDS.i);
...
int main(int argc, char **argv)
{
    ...
    pRDSLowPass = fir_init(rds_lpf_coefs, RDS_LPF_CUTOFF);
